...
class Spider:
    ...
    movie_image_folder = os.path.join(os.path.dirname(__file__), Spider.movie_image_path)
    star_image_folder = os.path.join(os.path.dirname(__file__), Spider.star_image_path)
    star_dict = {
        "姓名": "name",
        "国家": "country",
        "身高": "height",
        ...
        if star_soups is not None:
            for star_soup in star_soups:
                ...
                if res == 0:
                    ...
                    filepath = os.path.join(Spider.star_image_folder, img_url.split('/')[-1])
                    self.download_img(img_url, filepath)
                    star.image = os.path.join(Spider.star_image_path, img_url.split('/')[-1])
